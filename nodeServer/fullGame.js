class food {
    constructor(game, size, x, y) {
        ...
        this.x = x;
        this.y = y;
        this.ArrColor = ["#FF0000", "#FFFF00", "#00FF00", "#FF00FF", "#FFFFFF", "#00FFFF", "#7FFF00", "#FFCC00"];
        this.init();
    }

    init() {
        this.color = this.ArrColor[Math.floor(Math.random() * 99999) % this.ArrColor.length];
    }
...
class snake {
    ...
    init() {
        ...
        this.angle = 0;
        this.dx = Math.random() * this.game.MaxSpeed - Math.random() * this.game.MaxSpeed;
        this.dy = Math.random() * this.game.MaxSpeed - Math.random() * this.game.MaxSpeed;
        ...
        if (this.name != "HaiZuka") {
            ...
            if (this.time <= 0) {
                ...
                for (let i = 0; i < this.game.FOOD.length; i++) {
                    if (this.game.FOOD[i].size > this.game.getSize() / 10 && this.range(this.v[0], this.game.FOOD[i]) < minRange) {
                        minRange = this.range(this.v[0], this.game.FOOD[i]);
                        this.dx = this.game.FOOD[i].x - this.v[0].x;
                        this.dy = this.game.FOOD[i].y - this.v[0].y;
                    }
                }
                if (minRange < Math.sqrt(this.game.game_W * this.game.game_W + this.game.game_H * this.game.game_H))
                    this.time = 0;
                // console.log(minRange);

                while (Math.abs(this.dy) * Math.abs(this.dy) + Math.abs(this.dx) * Math.abs(this.dx) > this.game.MaxSpeed * this.game.MaxSpeed && this.dx * this.dy != 0) {
                    this.dx /= 1.1;
                    this.dy /= 1.1;
                }
                while (Math.abs(this.dy) * Math.abs(this.dy) + Math.abs(this.dx) * Math.abs(this.dx) < this.game.MaxSpeed * this.game.MaxSpeed && this.dx * this.dy != 0) {
    ...
    class Game {
        constructor() {
            ...
            this.XX = 0;
            this.YY = 0;
            this.names = [ "ab", "cd", "ef", "gh", "ij", "kl", "mn", "op", "qr", "st", "uv", "wx", "yz" ];
            this.canvas = null;
            this.context = null;
            this.init();
        }
                    ...
                    if (i != j) {
                        ...
                        if (!kt) {
                            for (let k = 0; k < this.mySnake[i].v.length; k += 5) {
                                this.FOOD[this.index] = new food(this, this.getSize() / (2 + Math.random() * 2), this.mySnake[i].v[k].x + Math.random() * this.mySnake[i].size / 2, this.mySnake[i].v[k].y + Math.random() * this.mySnake[i].size / 2);
                                this.FOOD[this.index++].value = 0.4 * this.mySnake[i].score / (this.mySnake[i].v.length / 5);
                                if (this.index >= this.FOOD.length)
                                    this.index = 0;
                            }
                            if (i != 0)
        ...
        isPoint(x, y) {
            return !(x - this.XX < -3 * this.getSize() ||
